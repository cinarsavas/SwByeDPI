...
Благодарность [romanvht](https://github.com/romanvht) за его [форк реализации Android-приложения над byedpi](https://github.com/romanvht/ByeByeDPI), которая вдохновила на создание данного проекта [вместе с iOS-приложением, работающим на данном модуле](./Example/Sources/ByeByeDPI)
...
3. [SwByeDPI](./Sources/SwByeDPI) - Расширенная обертка над ByeDPIC, которая базируется на ByeDPIKit'е. Содержит абстракции для конфигураций запуска, инструменты для создания URL-сессий под SOCKS-прокси, [предварительно добавленные наборы стратегий и доменов для тестирования и дальнейшего использования в byeDPI](./Assets), а также функционал по тестированию и аналитике стратегий и доменов
...
Генерация происходит через [скрипт-кодогенератор](./assets_codegen.sh) в ручном режиме, который определяет в [Assets](./Assets) все файлы с расширениями *.strategies и *.domains. **Примечание**: формат файлов списков един и для стратегий, и для доменов - по одной(-ому) на строку

## Example-приложение

В качестве [Example-приложения](./Example/Sources/ByeByeDPI) реализовано iOS-приложение (iOS 14+, SwiftUI). Проект может быть собран и под другие платформы (Экспериментальная поддержка для tvOS 17.0+ и macOS 11.0+)
...
Возможны 3 варианта сборки приложения:
...
- **ByeDPI SOCKS прокси-сервер с поддержкой работы в фоне (ByeDPIBg)** - Все то же самое, что и для варианта выше, но с поддержкой работы при свернутом приложении через бесконечно проигрываемое беззвучное аудио, которое не прерывает обычное аудио (mixWithOthers)
...
**Создание неподписанной сборки (AltStore/SideStore/TrollStore/LiveContainer и др.)**

2 схемы (ByeDPIBg, ByeByeDPI) собираются через xcodebuild и упаковываются в IPA через [xcodebuild_unsigned.sh](./xcodebuild_unsigned.sh)

Артефакты от результата выполнения скрипта находятся в [Releases](https://github.com/mIwr/SwByeDPI/releases/latest)
